<template>
  ...
  <div class="search" :class="{focused: focused}">
    ...
    <div class="input-wrap" @click="goSearch">
      <input type="text" :placeholder="placeholder" v-model="query" @input="input" @confirm="confirm" >
    ...
    <div class="content">
      ...
      <!-- 结果 -->
      <scroll-view scroll-y class="result" v-if="resultList.length">
        <navigator :url="'/pages/goods/index?id='+item.goods_id" v-for="item in resultList" :key="item.goods_id">
            {{item.goods_name}}
        </navigator>
      </scroll-view>
    </div>
